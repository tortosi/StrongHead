...
if ((whPos = document.domain.indexOf(".dragonfire-bg.net")) != -1) {
...
$WH.g_setTooltipLevel = function (g, a, l) {
    ...
    g = g.replace(/<!--ppl(\d+):(\d+):(\d+):(\d+)-->\s*\d+/gi, function (p, m, o, q, r) {
        return "<!--ppl" + m + ":" + o + ":" + q + ":" + r + "-->" + Math.round(parseInt(q) + (Math.min(Math.max(a, m), o) - m) * r / 100)
    });
    ...
    if (e) {
        if (o && o.modified) {
            ...
            for (var f in c) {
                ...
                $(e).append('<input type="checkbox" id="known-' + f + '" />').append('<label for="known-' + f + '"><a rel="spell=' + f + "&know=" + f + '">' + g_spells[f]["name_" + Locale.getName()] + (g_spells[f]["rank_" + Locale.getName()] ? " (" + g_spells[f]["rank_" + Locale.getName()] + ")" : "") + "</a></label>").append("<br />");
...
$WH.g_getProfileIcon = function (d, e, a, g, c, b) {
    ...
    if (c) {
        return isNaN(c) ? c : "/profile=avatar" + (b ? "&size=" + b : "") + "&id=" + c + (b == "tiny" ? ".gif" : ".jpg")
    }
    if (!g_file_races[d] || !g_file_classes[e] || !g_file_genders[a] || !f[d] || !f[d][e] || (e == 6 && g < 55)) {
        return "inv_misc_questionmark"
    }
    if (d == 22 || d == 9) {
        return "race_" + g_file_races[d] + "_" + g_file_genders[a]
    }
