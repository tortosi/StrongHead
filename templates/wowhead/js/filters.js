...
function fi_submit(e) {
    ...
    if (c == 0 && !d.filter) {
        alert('Please fill some criteria.');
        return false
    }
    return true
}
function fi_initWeightedListview() {
    this._scoreMode = (this._upgradeIds ? 2 : 0);
...
function fi_filterUpgradeListview(c, b) {
    ...
    if ($WH.in_array(this._upgradeIds, c.id) != -1) {
        if (b < a) {
            this._upgradeRow++
        }
        return true
...
function fi_addUpgradeIndicator() {
    if (this._upgradeIds) {
        var d = fi_filterParamToJson($WH.g_parseQueryString(location.href.replace(/^.*?filter=/, "filter=")).filter).upg;
        for (var a = 0; a < this.data.length; ++a) {
            var c = this.data[a],
                b = d.replace(c.id, "").replace(/(^:*|:*$)/g, "").replace("::", ":");
            if ($WH.in_array(this._upgradeIds, c.id) != -1) {
                this.createIndicator($WH.sprintf(LANG.lvnote_upgradesfor, c.id, (7 - parseInt(c.name.charAt(0))), c.name.substr(1)), location.href.replace(";upg=" + d, (b ? ";upg=" + b : "")))
            }
        }
    }
}
    ...
    if (d.selectedIndex != d.i) {
        ...
        if (d.selectedIndex > 0) {
            if (d.form.elements.gm.selectedIndex == 0) {
                d.form.elements.gm.selectedIndex = 2
            }
            fi_resetCriterion($WH.ge("fi_weight"));
...
function fi_presetSave() {
    ...
    var c = $("#sdkgnsdkn436 option:selected").get(0),
        a = "",
        b = ((c._weights ? c._weights.id : 0) | 0),
        e = {
            id: b,
            name: a
        },
    ...
    if (d < 1 || (!(c._weights && c._weights.id) && g_user.weightscales && g_user.weightscales.length >= 5)) {
        return alert(LANG.message_weightscalesaveerror)
    }
    if (a = prompt(LANG.prompt_nameweightscale, $(c).text())) {
        var f = {
            save: 1,
            name: $WH.urlencode(a),
            scale: ""
        ...
        $.post("/account=weightscales", f, function (q) {
            if (q > 0) {
                ...
                e.id = q;
                g_user.weightscales.push(e);
                var m = $("#fi_presets select").get(0),
                    t = $("option[value=-1]", m).get(0);
                if (!t) {
                    t = $WH.ce("option");
                    t.value = -1;
                    $WH.ae(t, $WH.ct(LANG.ficustom));
                    $WH.aef(m, t);
                    $WH.aef(m, $("option", m).get(1))
                }
                t._presets = {
                    custom: {}
                };
                for (var l = 0, k = g_user.weightscales.length; l < k; ++l) {
                    t._presets.custom[g_user.weightscales[l].id] = g_user.weightscales[l]
                }
                t.selected = true;
                t.parentNode.onchange();
                var r = $("#sdkgnsdkn436 option[value=" + e.id + "]").get(0);
                if (r) {
                    r.text = e.name;
                    r.selected = true;
                    r.parentNode.onchange()
                }
...
function fi_scoreSockets(r) {
    ...
    if (this._upgradeIds && $WH.in_array(this._upgradeIds, r.id) != -1) {
